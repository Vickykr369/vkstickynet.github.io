let Open_Form = document.getElementById('registeration-btn');
let Registration_Form = document.getElementById('register_form');
let Close_Registration_form = document.getElementById('Close_registeration');
let Sign_Up_Naviagtor = document.getElementById('sign-up');
let Login_Navigator = document.getElementById('login');
let Login_Form = document.getElementById('login-form-container');
let Sign_Up_Form = document.getElementById('sign_up-form-container');
let Show_sign_up_password = document.getElementById('show-pass-word-btn');
let show_login_password = document.getElementById('show-login-password-btn');
let password_for_signUp = document.getElementById('user-password');
let password_for_login = document.getElementById('user-password-for-login');
let Alphabet = /^[A-Za-z_]+$/;
let Valid_email = /^[a-zA-Z0-9.! #$%&'*+/=? ^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/;
let valid_phone = /^\(?([0-9]{3})\)?[-. ]?([0-9]{3})[-. ]?([0-9]{4})$/;
let SignUp_username = document.getElementById('username');
let SignUp_email = document.getElementById('user-email');
let SignUp_phone_number = document.getElementById('phone-number');
let SignUp_password = document.getElementById('user-password');
let div_of_SignUp_password = document.getElementById('pass.value');
let Repeat_SignUp_password = document.getElementById('user-password-repeat');
let Login_username = document.getElementById('Username-for-login');
let Login_phone_number = document.getElementById('phone-number-for-login');
let Login_password = document.getElementById('user-password-for-login');
let Login_btn = document.getElementById('login-btn');
let signUp_btn = document.getElementById('sign_up-btn');
let S_Nameerror = document.getElementById('NameError');
let S_emailerror = document.getElementById('EmailError');
let S_Phoneerror = document.getElementById('PhoneError');
let S_passworderror = document.getElementById('errpass2');
let S_errorRepeatpassword = document.getElementById('repeatError');
Close_Registration_form.addEventListener('click', () => {
    Login_Navigator.style.backgroundColor = "white";
    Login_Form.style.display = "none";
    Registration_Form.style.transform = "scale(0)";
    Sign_Up_Form.style.display = "block";
    Registration_Form.style.opacity = "0"
});

Open_Form.addEventListener('click', () => {
    Registration_Form.style.transform = "scale(1)";
    Sign_Up_Naviagtor.style.backgroundColor = "rgba(24, 207, 55, 0.945)";
    Registration_Form.style.opacity = "100";
    Registration_Form.style.borderRadius = "0"
});
